-- Invoice hotfix: signed decimal amounts, percentage discounts, eSolver reference.
-- No rewrites of historical invoice amounts. Legacy nonzero discounts should be
-- verified against original documents before resaving; old calculations ignored them.
BEGIN;
ALTER TABLE public.invoices ADD COLUMN esolver_registration_number text;
COMMENT ON COLUMN public.invoices.esolver_registration_number IS 'Optional external eSolver registration reference; entered manually, not globally unique.';
CREATE INDEX invoices_esolver_registration_idx ON public.invoices(esolver_registration_number);
ALTER TABLE public.invoice_lines DROP CONSTRAINT invoice_lines_discount_valid;
ALTER TABLE public.invoice_lines ADD CONSTRAINT invoice_lines_discount_valid CHECK(discount BETWEEN -100 AND 100) NOT VALID;
COMMENT ON COLUMN public.invoice_lines.discount IS 'Percentage: net = round(quantity * unit_price * (1 - discount / 100), 2). Negative percentage is a surcharge.';
...
DECLARE
 ...
 net numeric; vat numeric; total numeric; line_net numeric; line_vat numeric; qty numeric; price numeric; rate numeric; discount_percent numeric;
...
BEGIN
 ...
 IF length(coalesce(payload->>'esolver_registration_number',''))>120 THEN RAISE EXCEPTION 'Invalid eSolver reference' USING ERRCODE='23514'; END IF;
 ...
 IF net IS NULL OR vat IS NULL OR total IS NULL OR abs(net)>=1e12 OR abs(vat)>=1e12 OR abs(total)>=1e12 OR net<>round(net,2) OR vat<>round(vat,2) OR total<>round(total,2) OR net+vat<>total OR total='NaN'::numeric THEN RAISE EXCEPTION 'Invalid totals' USING ERRCODE='23514'; END IF;
 IF total<0 AND EXISTS(SELECT 1 FROM public.financial_allocations a JOIN public.financial_movements m ON m.id=a.movement_id WHERE a.invoice_id=invoice_save.invoice_id AND m.archived_at IS NULL) THEN RAISE EXCEPTION 'Rimuovere i pagamenti attivi prima di convertire in rettifica negativa' USING ERRCODE='23514'; END IF;
 ...
 UPDATE public.invoices SET invoice_type=payload->>'invoice_type',invoice_number=trim(payload->>'invoice_number'),
  ...
  esolver_registration_number=CASE WHEN payload ? 'esolver_registration_number' THEN NULLIF(trim(payload->>'esolver_registration_number'),'') ELSE esolver_registration_number END,
 ...
 FOR item IN SELECT value FROM jsonb_array_elements(payload->'lines') LOOP
  ...
  IF qty IS NULL OR price IS NULL OR abs(qty)>1e9 OR abs(price)>1e9 OR qty<>round(qty,3) OR price<>round(price,4) OR rate='NaN'::numeric OR rate<>round(rate,2) OR qty='NaN'::numeric OR price='NaN'::numeric OR (rate IS NULL AND COALESCE(item->>'vat_exempt_reason','')='') OR rate<0 OR rate>100 OR COALESCE(length(trim(item->>'description')),0)=0 THEN RAISE EXCEPTION 'Invalid line' USING ERRCODE='23514'; END IF;
  discount_percent=coalesce(NULLIF(item->>'discount','')::numeric,0);
  IF discount_percent NOT BETWEEN -100 AND 100 OR discount_percent<>round(discount_percent,4) OR discount_percent='NaN'::numeric THEN RAISE EXCEPTION 'Invalid discount percentage' USING ERRCODE='23514'; END IF;
  line_net=round(qty*price*(1-discount_percent/100),2); line_vat=CASE WHEN COALESCE(item->>'vat_exempt_reason','')<>'' THEN 0 ELSE round(line_net*rate/100,2) END;
  ...
  INSERT INTO public.invoice_lines(id,invoice_id,position,description,quantity,unit_price,vat_rate,vat_exempt_reason,amount_net,amount_vat,amount_total,project_id,unit,discount,notes)
  VALUES(detail_id,invoice_id,cardinality(line_ids),item->>'description',qty,price,rate,NULLIF(item->>'vat_exempt_reason',''),line_net,line_vat,line_net+line_vat,NULLIF(item->>'project_id','')::uuid,NULLIF(item->>'unit',''),discount_percent,NULLIF(item->>'notes',''))
  ON CONFLICT(id) DO UPDATE SET position=EXCLUDED.position,description=EXCLUDED.description,quantity=EXCLUDED.quantity,unit_price=EXCLUDED.unit_price,vat_rate=EXCLUDED.vat_rate,vat_exempt_reason=EXCLUDED.vat_exempt_reason,amount_net=EXCLUDED.amount_net,amount_vat=EXCLUDED.amount_vat,amount_total=EXCLUDED.amount_total,project_id=EXCLUDED.project_id,unit=EXCLUDED.unit,discount=EXCLUDED.discount,notes=EXCLUDED.notes;
 ...
 FOR item IN SELECT value FROM jsonb_array_elements(payload->'installments') LOOP
  ...
  detail_id=COALESCE(detail_id,gen_random_uuid());
  IF detail_id=ANY(installment_ids) OR NULLIF(item->>'due_date','') IS NULL OR (item->>'amount')::numeric IS NULL OR (item->>'due_date')::date < DATE '1900-01-01' OR (item->>'amount')::numeric='NaN'::numeric OR (item->>'amount')::numeric=0 OR sign((item->>'amount')::numeric)<>sign(total) OR abs((item->>'amount')::numeric)>=1e12 OR (item->>'amount')::numeric<>round((item->>'amount')::numeric,2) OR jsonb_typeof(item->'paid') IS DISTINCT FROM 'boolean' THEN RAISE EXCEPTION 'Invalid installment' USING ERRCODE='23514'; END IF;
...
BEGIN
  ...
  -- All line fields, including new-line discount, are saved atomically by save_invoice.
