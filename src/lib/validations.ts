import { z } from "zod";
import { cents, roundMoney } from "./money";
...
const amount = z.number().finite().min(0).max(999999999999.99).refine(n => Math.abs(n * 100 - Math.round(n * 100)) < 0.0001, "Massimo due decimali");
...
export const projectStatusSchema = z.enum(["draft", "active", "suspended", "completed", "archived"]);
export const invoiceLineSchema = z.object({
  id: uuidSchema.optional(), project_id: nullableUuid, unit: optionalText, discount: amount.default(0), notes: optionalText,
  description: z.string().trim().min(1).max(2000), quantity: z.number().finite().positive().max(1e9).refine(n => Math.abs(n*1000-Math.round(n*1000))<0.0001, "Massimo tre decimali"),
  unit_price: z.number().finite().min(0).max(1e9).refine(n => Math.abs(n*10000-Math.round(n*10000))<0.0001, "Massimo quattro decimali"), vat_rate: rate,
  vat_exempt_reason: z.string().trim().max(500).nullable(),
  amount_net: amount, amount_vat: amount, amount_total: amount,
}).superRefine((v, ctx) => {
  const net = roundMoney(v.quantity * v.unit_price);
  const vat = v.vat_exempt_reason ? 0 : roundMoney(net * (v.vat_rate ?? 0) / 100);
...
export const projectFormSchema = z.object({
  ...
  customer_id: nullableUuid, project_manager_id: nullableUuid, legal_entity_id: nullableUuid,
...
export const invoiceSchema = z.object({
  ...
  else if (v.vat_rate !== null && cents(v.vat_amount) !== cents(roundMoney(v.amount_net * v.vat_rate / 100))) issue("IVA incoerente con aliquota e imponibile");
