...
export async function CommercialDetail({kind,id}:{kind:CommercialKind;id:string}){
 ...
 {isOrder&&progress.data&&<section className="app-card p-3 my-3"><h2 className="h5">Riepilogo ({record.currency})</h2><p className="mb-0">Ordinato: {formatMoney(progress.data.ordered_value,record.currency)} · Consegnato (proporzionale al totale riga, inclusi sconto e IVA): {formatMoney(progress.data.delivered_value,record.currency)} · Fatturato attribuito: {progress.data.invoiced_value==null?"Da attribuire":formatMoney(progress.data.invoiced_value,record.currency)} · Differenza: {progress.data.invoiced_value===null?"Non determinabile":formatMoney(Number(progress.data.ordered_value)-Number(progress.data.invoiced_value),record.currency)}</p></section>}
 <section className="app-card p-3 my-3"><h2 className="h5">Righe</h2><div className="table-responsive"><table className="table table-sm"><thead><tr><th>Descrizione</th><th>Quantità</th><th>Unità</th><th>Commessa</th>{isOrder?<><th>Prezzo</th><th>Sconto</th><th>IVA %</th><th>Totale</th><th>Consegnato</th><th>Residuo</th></>:<th>Ordine / riga</th>}<th>Note</th></tr></thead><tbody>{record.lines.map(l=><tr key={l.id}><td>{l.description}</td><td>{l.quantity}</td><td>{l.unit}</td><td>{l.project_id?<Link href={`/commesse/${l.project_id}`}>{projectNames.get(l.project_id)??"Commessa"}</Link>:"—"}</td>{isOrder?<><td>{formatMoney(l.unit_price,record.currency)}</td><td>{l.discount}</td><td>{l.vat_rate??"—"}</td><td>{formatMoney(l.amount_total,record.currency)}</td><td>{l.delivered_quantity}</td><td>{l.remaining_quantity}</td></>:<td>{lineOrders.data?.filter(o=>o.id===l.order_line_id).map(o=><Link key={o.id} href={`/ordini/${o.order_id}`}>{related.data?.find(r=>r.id===o.order_id)?.order_number??"Ordine"} · {o.description}</Link>)}</td>}<td>{l.notes}</td></tr>)}</tbody></table></div></section>
 <section className="app-card p-3 my-3"><h2 className="h5">{isOrder?"DDT collegati":"Ordini collegati"}</h2>{(related.data??[]).map(r=><div key={r.id}><Link href={`/${isOrder?"ddt":"ordini"}/${r.id}`}>{r.note_number??r.order_number}</Link> · {formatDate(r.note_date??r.order_date)}{r.archived_at&&" · Archiviato"}</div>)}{!otherIds.length&&<p className="text-muted">Nessun collegamento.</p>}</section>
 <InvoiceLinks kind={kind} record={record}/><CommercialDocuments kind={kind} record={record}/>
 <ContextAnomalies kind={kind} id={id}/><ContextWork kind={kind} id={id}/>
 <section className="app-card p-3 my-3"><h2 className="h5">Audit</h2><ul>{(events.data??[]).map((e:{id:string;created_at:string;user_id:string|null;entity_type:string;action:string})=><li key={e.id}>{formatDateTime(e.created_at)} · {e.entity_type} · {e.action} · {e.user_id??"Sistema"}</li>)}</ul></section>
 </>;
}
