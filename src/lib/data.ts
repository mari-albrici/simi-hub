...
export type ProjectSummary = { documents:number; invoices:number; supplier_invoices:number; customer_invoices:number; open_deadlines:number; overdue_deadlines:number; financial:Array<{kind:string;currency:string;original:number;settled:number;residual:number}> };
