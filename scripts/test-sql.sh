...
docker exec "$container" sh -c 'for file in /tmp/migrations/00[456789]_*.sql /tmp/migrations/010_*.sql /tmp/migrations/011_*.sql; do psql -U postgres -d phase0_upgrade -q -v ON_ERROR_STOP=1 -f "$file" || exit 1; done'
