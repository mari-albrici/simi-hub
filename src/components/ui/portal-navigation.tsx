...
function NavigationTree({ items, mobile, pathname }: { items: NavigationItem[]; mobile: boolean; pathname: string }) {
  ...
  return <nav aria-label="Navigazione principale" className="nav flex-column gap-1">{single("/dashboard")}{single("/commesse")}{group(0)}{single("/documenti")}{group(1)}{group(2)}{["/personale", "/report", "/impostazioni"].map(single)}</nav>;
}
