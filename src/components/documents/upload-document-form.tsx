...
export function UploadDocumentForm({options,values={},scopes,documentId}:{options:DocumentOptions;values?:Record<string,unknown>;scopes:string[];documentId?:string}){
 ...
 <div className="col-12"><button className="btn btn-dark" disabled={pending} aria-busy={pending}>{pending?<LoadingSpinner label="Caricamento documento…"/>:documentId?"Carica nuova versione":"Carica documento"}</button></div>
 </form>;
}
