...
export async function ProjectCycle({id}:{id:string}){
 ...
 <h3 className="h6">Ordini ({orders.count})</h3><div className="table-responsive"><table className="table table-sm"><thead><tr><th>Numero</th><th>Data</th><th>Controparte</th><th>Stato evasione</th><th>Quota commessa</th></tr></thead><tbody>{orders.rows.map(o=><tr key={o.id}><td><Link href={`/ordini/${o.id}`}>{o.order_number}</Link></td><td>{o.order_date}</td><td>{parties.data?.find(c=>c.id===o.counterparty_id)?.business_name}</td><td>{commercialLabels[o.fulfillment_status]??o.fulfillment_status}</td><td>{sums.has(o.id)?`${sums.get(o.id)} ${o.currency}`:"Non attribuito alle righe"}</td></tr>)}</tbody></table></div>{!orders.count&&<p className="text-muted">Nessun ordine.</p>}{orders.count>50&&<Link href={`/ordini?project=${id}`}>Tutti gli ordini</Link>}
 <h3 className="h6">DDT ({notes.count})</h3>{notes.rows.map(d=><div key={d.id}><Link href={`/ddt/${d.id}`}>{d.note_number}</Link> · {d.note_date} · {commercialLabels[d.direction]}</div>)}{!notes.count&&<p className="text-muted">Nessun DDT.</p>}{notes.count>50&&<Link href={`/ddt?project=${id}`}>Tutti i DDT</Link>}
 </section>;
}
