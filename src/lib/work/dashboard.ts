...
import { syncAnomalies } from "./data";
...
export async function getWorkDashboard(){
 ...
 const [mine,urgent,anomalies,critical,count]=await Promise.all([
 ...
 db.from("anomalies").select("id",{count:"exact",head:true}).eq("status","open"),
 ]);for(const r of [mine,urgent,anomalies,critical,count])checkDatabase(r.error);
 return {tasks:[...new Map([...(mine.data??[]),...(urgent.data??[])].map(t=>[t.id,t])).values()] as Pick<Task,"id"|"title"|"status"|"priority"|"due_date">[],anomalies:[...new Map([...(critical.data??[]),...(anomalies.data??[])].map(a=>[a.id,a])).values()].slice(0,4) as Pick<Anomaly,"id"|"title"|"status"|"severity"|"due_date">[],count:count.count??0};
}
