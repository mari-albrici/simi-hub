...
export async function InvoiceCycle({id}:{id:string}){
 ...
 {write&&<form action={commercialRelationAction} className="d-flex gap-2"><input type="hidden" name="kind" value={kind}/><input type="hidden" name="invoice_id" value={id}/><input type="hidden" name="return_invoice" value="1"/><select aria-label={isOrder?"Ordine compatibile":"DDT compatibile"} name="record_id" className="form-select" required><option value="">Seleziona {isOrder?"ordine":"DDT"}</option>{choices.map(row=><option key={row.id} value={row.id}>{row.order_number??row.note_number} · {row.order_date??row.note_date}</option>)}</select>{isOrder&&<input name="amount" aria-label={`Importo attribuito ${i.currency}`} placeholder={`Importo attribuito ${i.currency} (facoltativo)`} type="number" min="0" step="0.01" className="form-control"/>}<SubmitButton className="btn btn-outline-primary">Collega</SubmitButton></form>}</div>;
 }));
